...
abstract class StageVerticle : AbstractVerticle() {
    lateinit var definition: StageDefinition
    var namespace: String = Constants.DEFAULT_NAMESPACE
    val verticleConsumers: MutableList<MessageConsumer<*>> = CopyOnWriteArrayList()

    override fun start() {
        super.start()
        this.definition = StageDefinition.fromJsonObject(config().getJsonObject("definition"))
    ...
    fun subscribe(laneName: String, handler: Handler<Message<KEvent>>) {
        getInputLane(laneName)?.let {
            val messageConsumer = vertx.eventBus().consumer(getEventBusAddress(it), handler)
            verticleConsumers.add(messageConsumer)
        }
    }
